...
class HardwarePage(QWizardPage):
    ...
    def _refresh_size(self) -> None:
        ...
        self.size_label.setText(
            f"将下载 <b>{len(ids)}</b> 个语言包，合计 <b>{human_size(total)}</b>"
            + ("（已安装的会自动跳过）" if total else "")
        )
...
class FirstRunWizard(QWizard):
    def __init__(self, config: AppConfig | None = None) -> None:
        ...
        self.setWindowTitle("听·显·译 — 首次运行向导")
        self.resize(760, 620)
    ...
    def _apply(self) -> None:
        ...
        c.asr.latency_preset = {"low": "realtime", "mid": "balanced", "high": "accurate"}.get(
            tier, "balanced"
        )
        try:
        ...
        try:
            c.save()
            log.info("首次运行向导完成，配置已保存")
        except Exception as exc:  # noqa: BLE001
            log.error("保存配置失败: %s", exc)


def run_wizard(config: AppConfig | None = None) -> int:
    """跑向导。返回 QDialog 结果码。"""
